...
void main(void) {
  ...
  // ------------------------------------------------------------------------------  
  vec4  leg   = get_point(legs_data,   legs_dimensions,   float(gl_FragCoord.x));
  
  vec4  point = get_point(points_data, points_dimensions, float(gl_FragCoord.y));
  
  vec2  p = nearest_point_on_line(point.x, point.y, leg.r, leg.g, leg.b, leg.a);

  float d = earth_distance(point.x, point.y, p.x, p.y);
  float m = distance_to_segment(point.x, point.y, leg.r, leg.g, leg.b, leg.a);

  vec4  f = encode_float(min(m, d));
  gl_FragColor = f;
}
